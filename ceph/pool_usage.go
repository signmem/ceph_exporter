...
func NewPoolUsageCollector(exporter *Exporter) *PoolUsageCollector {
	...
	return &PoolUsageCollector{
		...
		ReadIO: prometheus.NewDesc(fmt.Sprintf("%s_%s_rd", cephNamespace, subSystem), "Total read I/O calls for the pool",
			poolLabel, labels,
		),
		ReadBytes: prometheus.NewDesc(fmt.Sprintf("%s_%s_rd_bytes", cephNamespace, subSystem), "Total read throughput for the pool",
			poolLabel, labels,
		),
		WriteIO: prometheus.NewDesc(fmt.Sprintf("%s_%s_wr", cephNamespace, subSystem), "Total write I/O calls for the pool",
			poolLabel, labels,
		),
		WriteBytes: prometheus.NewDesc(fmt.Sprintf("%s_%s_wr_bytes", cephNamespace, subSystem), "Total write throughput for the pool",
			poolLabel, labels,
		),
	}
}
